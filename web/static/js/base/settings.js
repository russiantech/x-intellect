class Settings { 
    ...
    _init() { 
        ...
        this._mergeOverrides(), this._mergeUrlParameters(), 
        this._modifyLinksCarryParams(), this._setAttributes(), 
        this._setActiveOptions(), this._addListeners(), this._setVisibility() 
    }

    _mergeAttributesFromStorage() { for (const t in this.settings.attributes) 
    ...
    _mergeOverrides() { const t = this._getOverrideJSON(); if (!t) return; const e = Object.assign(this.settings.attributes, t.attributes); this.settings = Object.assign(this.settings, t), this.settings.attributes = e } 
    
    _mergeOverridePrefix() { const t = this._getOverrideJSON(); t && t.storagePrefix && (this.settings.storagePrefix = t.storagePrefix) } 
    
    _getOverrideJSON() { const t = document.documentElement.getAttribute("data-override"); if (!t) return null; return JSON.parse(t) }
    
    _mergeUrlParameters() { 
        const t = window.location.search, 
        e = decodeURIComponent(t), i = new URLSearchParams(e), 
        s = JSON.parse(i.get("params")); 
        if (s && s.attributes) { const t = Object.assign(this.settings.attributes, s.attributes); 
    ...
    if (e && !e.getAttribute("href").includes("#")) { const t = window.location.search; document.location = e.getAttribute("href") + t } t.preventDefault() })) } 
    
    _setAttributes() { for (const t in this.settings.attributes) document.documentElement.setAttribute(this.attributeOptions[t].attribute, this.settings.attributes[t]) } 
    
    _setActiveOptions() { this._clearActiveOptions(), document.querySelectorAll(this.optionSelector).forEach(((t, e) => { t.dataset.value === this.settings.attributes[t.dataset.parent] && t.classList.add("active") })) } 
    
    _clearActiveOptions() { document.querySelectorAll(this.optionSelector).forEach(((t, e) => { t.classList.remove("active") })) } 
    
    _addListeners() { 
    document.querySelectorAll(this.optionSelector).forEach(((t, e) => { t.addEventListener("click", this._onOptionClick.bind(this)) })), 
    ...
    _onOptionClick(t) { t.preventDefault(); const e = t.currentTarget,  i = e.dataset.value, s = e.dataset.parent; this.updateAttribute(s, i) } 
    
    _setVisibility() { 
        this.settings.showSettings || (document.getElementById("settings") && 
        ...
        _onLightDarkModeClick() { 
            let t = this.settings.attributes.color; t = t.includes("light") ? t.replace("light", "dark") : t.replace("dark", "light"), 
            this.updateAttribute("color", t) 
    } 

    _onPinButtonClick() { 
        let t = this.settings.attributes.behaviour; 
        t = "pinned" === t ? "unpinned" : "pinned", 
    ...
    updateAttribute(t, e) { 
        ...
        this.settings.attributes[t] !== e && (
            ...
            this.attributeOptions[t].update && document.documentElement.setAttribute(this.attributeOptions[t].attribute, e), 
