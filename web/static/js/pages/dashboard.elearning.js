class ElearningDashboard {
    ...
    _initProgressBars() { 
        document.querySelectorAll(".progress-bar").forEach(
            ( t => { const e = t.getAttribute("aria-valuenow"); t.style.width = e + "%" })
            )
         }

    _initAchievementsCarousel() {
        document.querySelector("#glideAchievements") && 
        new GlideCustom( document.querySelector("#glideAchievements"), 
            {
            gap: 0, rewind: !1, bound: !0, perView: 6, 
            breakpoints: { 600: { perView: 1 }, 1400: { perView: 2 }, 1900: { perView: 4 }, 3840: { perView: 5 }  }
        ...
        if (document.getElementById("timeChart")){
            ...
            this._timeChart = new Chart( t, {
                ...
                data: {
                    datasets: [
                        {
                        label: "russian coders", 
                        data: [360, 475, 780],
                        backgroundColor: 
                        [
                            "rgba(" + Globals.primaryrgb + ", 0.1)", 
                            "rgba(" + Globals.secondaryrgb + ", 0.1)", 
                            "rgba(" + Globals.quaternaryrgb + ", 0.1)"
                        ],
                        borderColor: [ Globals.primary, Globals.secondary, Globals.quaternary ]
                    }
                ], 
                labels: ["Lessons", "Badges", "Quizzes"], 
                    ...
                    tooltips: {
                        ...
                        custom: function (t) {
                            ...
                            if (0 !== t.opacity) {
                                ...
                                t.yAlign ? e.classList.add(t.yAlign) : e.classList.add("no-transform"), t.body) {
                                    var a = this, 
                                    r = t.dataPoints[0].index, 
                                    i = e.querySelector(".icon");
                                    i.style = "color: " + t.labelColors[0].borderColor, 
                                    i.setAttribute("data-acorn-icon", 
                                    a._data.icons[r]),
    ...
    _initBarrating() {
        jQuery().barrating && jQuery(".rating").each(
            (function () {
            ...
            jQuery(this).barrating({ 
                initialRating: t, readonly: e, 
                showValues: r, showSelectedRating: a, 
